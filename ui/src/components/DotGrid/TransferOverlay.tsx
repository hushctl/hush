  ...
  return (
    <>
      ...
      {activeTransfers.map(t => {
        ...
        const CARD_W = 220
        const CARD_H = 80
        const cardX = Math.max(4, Math.min(size.w - CARD_W - 4, midX - CARD_W / 2))
        const cardY = Math.max(4, Math.min(size.h - CARD_H - 4, midY - CARD_H / 2))
        ...
        return (
          ...
          >
            <div className="truncate text-foreground">{t.projectName} / {t.branch}</div>
