...
function buildReboardingText(worktrees: WorktreeInfo[]): string | null {
  ...
  const parts: string[] = []
  if (needsYou > 0) parts.push(`${needsYou} ${needsYou === 1 ? 'worktree needs' : 'worktrees need'} your attention`)
  if (running > 0) parts.push(`${running} running`)
  if (failed > 0) parts.push(`${failed} failed`)
  if (parts.length === 0) parts.push(`${worktrees.length} ${worktrees.length === 1 ? 'worktree' : 'worktrees'} idle`)
  return parts.join(' · ')
}
...
export function DotGrid() {
  ...
  const [hovered, setHovered] = useState<{ wt: WorktreeInfo; pos: WorktreePosition } | null>(null)
  ...
  const handleDotClick = useCallback((wt: WorktreeInfo) => {
    openPane(wt.id)
  }, [openPane])

  const handleProjectLabelClick = useCallback((projectId: string) => {
      ...
      <div ref={containerRef} className="relative flex-1 overflow-hidden">
        ...
        >
          ...
          {wtPositions.map(pos => {
            ...
            const isFirstInProject = projectFirstDot.get(wt.project_id) === pos.worktreeId
        ...
        {wtPositions.map(pos => {
          ...
          return (
            <button
              ...
              onMouseEnter={() => { cancelHide(); setHovered({ wt, pos }) }}
        ...
        {hovered && (
          <div
            ...
            style={{
              left: Math.min(hovered.pos.x + hovered.pos.dotSize / 2 + 8, w - 300),
              top: Math.min(hovered.pos.y - 40, Math.max(0, h - 260)),
            }}
            onMouseEnter={cancelHide}
            onMouseLeave={scheduleHide}
          >
            <DetailCard worktree={hovered.wt} onOpen={() => { openPane(hovered.wt.id); setHovered(null) }} />
          </div>
        )}
        ...
        {wtList.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
            ...
            <p className="text-xs text-muted-foreground mt-1">Use the command bar below to get started.</p>
          </div>
        )}
      </div>
    </div>
