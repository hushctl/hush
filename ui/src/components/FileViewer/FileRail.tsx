...
export function FileRail({ worktreeId, className = '' }: Props) {
  ...
  if (fileContent) {
    return (
      <div className={`flex flex-col overflow-hidden text-xs font-mono ${className}`}>
        {/* File viewer header */}
        <div className="flex items-center gap-2 px-2 py-1.5 border-b border-border shrink-0">
          ...
          <span className="truncate text-muted-foreground" title={fileContent.path}>
            {fileContent.path}
          </span>
  ...
  return (
    <div className={`flex flex-col overflow-hidden text-xs font-mono ${className}`}>
      <div className="px-2 py-1.5 border-b border-border shrink-0">
        <span className="text-xs uppercase tracking-wider text-muted-foreground">changes</span>
      </div>

      {!gitStatus || !hasAny ? (
        <div className="flex-1 flex items-center justify-center px-3">
          <span className="text-muted-foreground text-xs">— clean working tree —</span>
        </div>
      ) : (
        <div className="flex-1 overflow-auto">
          {sections.map(section => {
            ...
            return (
              <div key={section}>
                <div className="px-2 py-1 uppercase tracking-wider text-muted-foreground" style={{ fontSize: '10px' }}>
                ...
                {files.map(file => (
                  ...
                  >
                    <span
                      ...
                      style={{ color: SECTION_COLORS[section], fontSize: '10px' }}
                    >
