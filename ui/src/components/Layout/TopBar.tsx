...
export function TopBar() {
  ...
  return (
    <div
      data-testid="top-bar"
      className="flex items-center gap-2 px-3 h-10 border-b border-border bg-background shrink-0"
    >
      {/* Back to grid */}
      <Button
      ...
      {panels.length > 0 && (
        <>
          ...
          <Button
            ...
            title={
              autoTidy
                ? "Auto-tidy on — click to disable"
                : "Auto-tidy off — click to enable"
            }
          >
            <span
      ...
      <div className="flex items-center gap-1">
        ...
        <Tooltip>
          ...
          <TooltipContent className="rounded-none text-xs font-mono">
            {connectedCount}/{totalCount}{" "}
            {totalCount === 1 ? "daemon" : "daemons"} connected
            {daemonList.map((d) => (
              <div key={d.id} className="mt-0.5">
                <span style={{ color: d.connected ? "#22c55e" : "#ef4444" }}>
                  ■
                </span>{" "}
                {d.name}
              </div>
