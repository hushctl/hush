...
export function CommandBar() {
  ...
  const [setupStep, setSetupStep] = useState<
    "project" | "worktree" | "daemon" | null
  >(null);
  const [setupData, setSetupData] = useState({
    projectPath: "",
    projectName: "",
    branch: "",
    daemonUrl: "wss://localhost:9111/ws",
    daemonName: "",
    machineId: "",
  ...
  async function handleSubmit() {
    ...
    if (intent.kind === "unknown" && modelStatus === "ready") {
      ...
      try {
        ...
        const gemmaResult = await classify(text, {
    ...
    if (result.ok) {
      ...
      setHint(
        result.error ??
          'unknown intent — try "pull up <project>" or "back to grid"',
      );
    }
  }

  function dispatchIntent(intent: IntentResult): {
    ok: boolean;
    error?: string;
  } {
    switch (intent.kind) {
      ...
      case "pull_up": {
        if (intent.worktreeIds.length === 0)
          return { ok: false, error: "no matching worktrees" };
      ...
      case "close": {
        if (intent.worktreeIds.length === 0)
          return { ok: false, error: "no matching worktrees to close" };
        for (const id of intent.worktreeIds) closePane(id);
        return { ok: true };
      }

      case "show_needs_me": {
        const ids = Object.values(worktrees)
          .filter((w) => w.status === "needs_you")
          .map((w) => w.id);
        if (ids.length === 0)
          return { ok: false, error: "nothing needs you right now" };
  ...
  const placeholder =
    layoutMode === "grid"
      ? 'workspace intent — "pull up my-project", "show me what needs me"…'
      : '"close my-project", "back to grid", "pull up another-project"…';

  const connectedCount = Object.values(daemons).filter(
    (d) => d.connected,
  ).length;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <span className="text-xs font-mono text-muted-foreground shrink-0 flex items-center gap-1">
          <span
            ...
            style={{
              backgroundColor: connectedCount > 0 ? "#22c55e" : "#ef4444",
            }}
          />
          {connectedCount}/{Object.keys(daemons).length}{" "}
          {Object.keys(daemons).length === 1 ? "daemon" : "daemons"}
        </span>
        {value.trim() && (
          <Button
        ...
        <Button
          ...
          className="rounded-none shadow-none font-normal shrink-0"
          onClick={() => {
            const firstConnected = Object.values(daemons).find(
              (d) => d.connected,
            );
            setSetupData((d) => ({
              ...d,
              machineId: firstConnected?.id ?? targetMachineId(),
            }));
            setShowSetup((v) => !v);
      ...
      {/* Gemma model status — loading bar (first load only) or classifying spinner.
          Hidden entirely when AI intent is disabled (default). */}
      {modelStatus === "loading" && (
        <div className="px-3 pb-2 space-y-1">
          <div className="flex items-center justify-between text-xs font-mono text-muted-foreground">
            <span>
              loading intent model
              {modelProgressFile ? ` (${shortFile(modelProgressFile)})` : ""}…
            </span>
      ...
      {hint && (
        <div
          ...
          className="px-3 pb-2 text-xs font-mono text-amber-500"
        >
          {hint}
        </div>
      )}
      ...
      {pendingCreate && (
        <div className="border-t border-border px-3 py-2 bg-muted flex items-center gap-3 text-xs font-mono">
          <span className="text-muted-foreground shrink-0">
            <span className="text-amber-500">directory not found:</span>{" "}
            {pendingCreate.path}
          </span>
      ...
      {showSetup && (
        <div
          ...
          className="border-t border-border px-3 py-2 bg-muted space-y-2"
        >
          {setupStep === "project" && (
            <div className="space-y-2">
              {Object.values(daemons).filter((d) => d.connected).length >=
                2 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono text-muted-foreground shrink-0">
                    daemon:
                  </span>
                  {Object.values(daemons)
                    ...
                    .map((d) => (
                      <button
                        ...
                        onClick={() =>
                          setSetupData((sd) => ({ ...sd, machineId: d.id }))
                        }
                      >
                        {d.name}
                      </button>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSetupData((d) => ({ ...d, projectPath: e.target.value }))
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter") handleRegisterProject();
                  }}
                  autoFocus
                />
                <input
                  ...
                  onChange={(e) =>
                    setSetupData((d) => ({ ...d, projectName: e.target.value }))
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter") handleRegisterProject();
                  }}
          ...
          {setupStep === "worktree" && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <input
                ...
                onChange={(e) =>
                  setSetupData((d) => ({ ...d, branch: e.target.value }))
                }
                onKeyDown={(e) => {
              ...
              {(
                ...
                <button
                  ...
                  onClick={() =>
                    setSetupData((d) => ({ ...d, permissionMode: mode }))
                  }
                >
                  {mode}
                </button>
          ...
          {setupStep === "daemon" && (
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setSetupData((d) => ({ ...d, daemonUrl: e.target.value }))
                }
                onKeyDown={(e) => {
                  if (e.key === "Enter") handleAddDaemon();
                }}
                autoFocus
              />
              <input
                ...
                onChange={(e) =>
                  setSetupData((d) => ({ ...d, daemonName: e.target.value }))
                }
                onKeyDown={(e) => {
                  if (e.key === "Enter") handleAddDaemon();
                }}
