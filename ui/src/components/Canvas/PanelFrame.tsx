...
export function PanelFrame({ panel }: Props) {
  ...
  const proj = wt
    ...
    : panel.kind === 'worktree_list' ? projects[panel.targetId] : null
  const color = wt ? statusColor(wt.status) : null

  const kindLabel = panel.kind === 'terminal' ? 'terminal' : panel.kind === 'shell' ? 'shell' : panel.kind === 'file_rail' ? 'files' : 'worktrees'
  const title = wt
  ...
  const onResizePointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>, edge: Edge) => {
    e.stopPropagation()
    e.currentTarget.setPointerCapture(e.pointerId)
    resizeStart.current = {
  ...
  const onResizePointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (edge.includes('w')) { nw = Math.max(200, pw - dx); nx = px + pw - nw }
    if (edge.includes('n')) { nh = Math.max(120, ph - dy); ny = py + ph - nh }
  ...
  function makeHandle(edge: Edge, style: React.CSSProperties) {
    ...
    return (
      <div
        key={edge}
        style={{ ...style, position: 'absolute', cursor: cursors[edge], zIndex: 10 }}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={color ? { borderLeftColor: color, borderLeftWidth: 2 } : undefined}
          ...
          {panel.kind === 'terminal' && (
            <>
              <button
                ...
                title="Open shell"
                onClick={() => openPanel({ kind: 'shell', targetId: panel.targetId })}
              >
                $
              </button>
              <button
                ...
                onClick={() => openPanel({ kind: 'file_rail', targetId: panel.targetId })}
              >
                ◫
              </button>
            </>
      ...
      <div className="flex-1 overflow-hidden min-h-0 relative">
        ...
        {panel.kind === 'worktree_list' && <WorktreeListPanel projectId={panel.targetId} />}
      </div>

      {/* Edge resize handles */}
      {makeHandle('n', { top: 0, left: CORNER, right: CORNER, height: EDGE })}
      {makeHandle('s', { bottom: 0, left: CORNER, right: CORNER, height: EDGE })}
