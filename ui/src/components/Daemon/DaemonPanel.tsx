...
export function DaemonPanel() {
  ...
  const peers = Object.values(daemons).filter(d => d.id !== machineId && d.connected)
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span
            ...
            style={{ backgroundColor: daemon.connected ? '#22c55e' : '#ef4444' }}
          />
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* Memory pulse */}
        <div className="px-4 py-3 border-b border-border">
          <div className="text-xs font-mono text-muted-foreground uppercase tracking-wide mb-2">memory</div>
          <MemoryPulse alert={memoryAlert} samples={samples} connected={daemon.connected} />
        </div>
          ...
          <div className="px-4 py-3 border-b border-border">
            ...
            <div className="flex flex-wrap gap-2">
              {daemonWorktrees.map(wt => (
                <WorktreeDot key={wt.id} worktree={wt} onOpen={() => openPane(wt.id)} />
              ))}
            </div>
            {/* Status summary */}
        ...
        {/* Peer mesh */}
        {(peers.length > 0 || Object.values(daemons).filter(d => d.id !== machineId).length > 0) && (
          <div className="px-4 py-3 border-b border-border">
            <div className="text-xs font-mono text-muted-foreground uppercase tracking-wide mb-2">peers</div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs font-mono border border-foreground px-2 py-1">{daemon.name}</span>
              {peers.length > 0 && (
                <>
              ...
              {peers.length === 0 && (
                <span className="text-xs font-mono text-muted-foreground">no connected peers</span>
              )}
            </div>
          </div>
        )}
        ...
        <div className="px-4 py-3 border-b border-border">
          ...
          {trustExpanded && (
            <div className="mt-2 space-y-1">
              <div className="text-xs font-mono text-muted-foreground">machine id</div>
              <div className="text-xs font-mono break-all">{machineId}</div>
              <div className="text-xs font-mono text-muted-foreground mt-2">address</div>
...
function WorktreeDot({ worktree, onOpen }: { worktree: WorktreeInfo; onOpen: () => void }) {
