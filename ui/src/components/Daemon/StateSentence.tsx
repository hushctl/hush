...
export function StateSentence({
  ...
  if (memoryAlert?.level === "critical") {
    parts.push(
      `${daemon.name} is under severe memory pressure — ${fmtBytes(memoryAlert.availableBytes)} free.`,
    );
  } else if (memoryAlert?.level === "warning") {
    parts.push(
      `${daemon.name} is running hot — memory tight at ${fmtBytes(memoryAlert.availableBytes)} free.`,
    );
  } else if (running > 0) {
  ...
  if (running > 0) {
    parts.push(
      `${running} worktree${running !== 1 ? "s" : ""} running${needsYou > 0 ? `, ${needsYou} waiting on you` : ""}.`,
    );
  } else if (total > 0) {
  ...
  return <p className="text-sm font-normal leading-snug">{parts.join(" ")}</p>;
}
