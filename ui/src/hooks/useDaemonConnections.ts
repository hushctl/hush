...
export function useDaemonConnections() {
  ...
  const sendersRef = useRef<Map<string, (msg: ClientMessage) => void>>(
    new Map(),
  );
  ...
  const send = useCallback((machineId: string, msg: ClientMessage) => {
    ...
    if (fn) {
      ...
      console.warn(
        `[useDaemonConnections] no connection for machine ${machineId}`,
      );
    }
  }, []);
  ...
  const wsMap = useRef<
    Map<
      string,
      { ws: WebSocket; machineId: string; retry: number; unmounted: boolean }
    >
  >({} as never);
