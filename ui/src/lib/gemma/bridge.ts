 ...
 *
 * The worker is created lazily — only when startModelLoad() is called.
 * This means the 22MB ONNX WASM runtime is never fetched unless the
 * AI intent feature is explicitly enabled via VITE_ENABLE_AI_INTENT=true.
 */
...
const pending = new Map<
  string,
  { resolve: (r: GemmaResult) => void; reject: (e: Error) => void }
>();
let reqCounter = 0;
...
/** Create the worker lazily — avoids fetching WASM until actually needed. */
    ...
    switch (msg.type) {
      ...
      case "result": {
        ...
        if (entry) {
          ...
          entry.resolve(parseRaw(msg.raw as string));
        }
        break;
      }
      case "result_error": {
        ...
        if (entry) {
          pending.delete(msg.id as string);
          entry.reject(new Error(msg.message as string));
        }
        break;
      }
    }
...
export async function classify(
  ...
  return new Promise((resolve, reject) => {
    ...
    ensureWorker().postMessage({ type: "classify", id, text, ctx });
  });
}
...
function parseRaw(raw: string): GemmaResult {
  ...
  if (!match)
    return { kind: "unknown", reason: `no JSON in: ${raw.slice(0, 80)}` };
  let parsed: Record<string, string>;
  try {
    parsed = JSON.parse(match[0]);
  } catch {
    return { kind: "unknown", reason: `bad JSON: ${match[0]}` };
  }
  ...
  switch (parsed.kind) {
    ...
    case "pull_up":
      return { kind: "pull_up", target: parsed.target ?? "" };
    case "close":
      return { kind: "close", target: parsed.target ?? "" };
    case "tree":
      return { kind: "tree", target: parsed.target ?? "" };
    case "new_worktree":
      return {
        kind: "new_worktree",
    ...
    case "delete_worktree":
      return { kind: "delete_worktree", target: parsed.target ?? "" };
    case "inspect_daemon":
      return { kind: "inspect_daemon", target: parsed.target ?? "" };
    default:
      return { kind: "unknown", reason: `unknown kind: ${parsed.kind}` };
  }
}
