...
export function parseIntent(input: string, ctx: IntentContext): IntentResult {
  ...
  if (text.startsWith("daemon ") || text.startsWith("inspect ")) {
    const target = text.startsWith("daemon ")
      ? text.slice(7).trim()
      : text.slice(8).trim();
  ...
  if (text.startsWith("tree ")) {
    ...
    if (!project)
      return { kind: "unknown", reason: `no project matching "${target}"` };
  ...
  if (text.startsWith("new worktree ")) {
    ...
    if (!project)
      return { kind: "unknown", reason: "no project to create worktree in" };
    return { kind: "new_worktree", projectId: project.id, branch };
  }

  // delete worktree <branch|project/branch>
  if (
    text.startsWith("delete worktree ") ||
    text.startsWith("remove worktree ")
  ) {
    const target = text.replace(/^(delete|remove) worktree\s+/, "");
    const wt = resolveWorktreeRef(target, ctx);
    if (!wt)
      return { kind: "unknown", reason: `no worktree matching "${target}"` };
  ...
  if (text.startsWith("move ") && text.includes(" to ")) {
    ...
    if (toIdx !== -1) {
      ...
      if (!daemon)
        return { kind: "unknown", reason: `no daemon matching "${destName}"` };
      ...
      if (target.includes("/")) {
        ...
        if (!wt)
          return {
            kind: "unknown",
            reason: `no worktree matching "${target}"`,
          };
        return {
          kind: "move_worktree",
          worktreeId: wt.id,
          destMachineId: daemon.id,
        };
      }
  ...
  if (text.startsWith("pull up ")) {
    ...
    for (const t of targets) {
      ...
      if (wt) {
        ...
        if (proj) {
          const projectWts = Object.values(ctx.worktrees).filter(
            (w) => w.project_id === proj.id,
          );
    ...
    if (ids.length === 0)
      return { kind: "unknown", reason: `no worktrees matching "${rest}"` };
...
function findDaemon(
  ...
  for (const d of Object.values(daemons)) {
    if (d.name.toLowerCase() === lower || d.id.toLowerCase() === lower)
      return d;
  }
  for (const d of Object.values(daemons)) {
    if (
      d.name.toLowerCase().startsWith(lower) ||
      d.id.toLowerCase().startsWith(lower)
    )
      return d;
  }
  return null;
...
function resolveWorktreeRef(
  ...
  if (lower.includes("/")) {
    ...
    return (
      Object.values(ctx.worktrees).find(
        (w) =>
          w.project_id === project.id && w.branch.toLowerCase() === branchName,
      ) ?? null
    );
  }
  ...
  if (!project) return null;
  return (
    Object.values(ctx.worktrees).find((w) => w.project_id === project.id) ??
    null
  );
}
 ...
 */
export function resolveGemmaResult(
  r: GemmaResult,
  ctx: IntentContext,
): IntentResult {
  switch (r.kind) {
    ...
    case "show_needs_me":
      return { kind: "show_needs_me" };
    case "unknown":
      return { kind: "unknown", reason: r.reason };
    ...
    case "pull_up": {
      ...
      if (!wt) {
        ...
        if (proj) {
          const wts = Object.values(ctx.worktrees).filter(
            (w) => w.project_id === proj.id,
          );
          if (wts.length > 0)
            return { kind: "pull_up", worktreeIds: [wts[0].id] };
        }
        return {
          kind: "unknown",
          reason: `no worktree matching "${r.target}"`,
        };
      }
      return { kind: "pull_up", worktreeIds: [wt.id] };
    }
    ...
    case "tree": {
      const proj = findProject(r.target, ctx);
      if (!proj)
        return { kind: "unknown", reason: `no project matching "${r.target}"` };
    ...
    case "inspect_daemon": {
      ...
      if (!d)
        return { kind: "unknown", reason: `no daemon matching "${r.target}"` };
...
function resolveAllMatching(ref: string, ctx: IntentContext): string[] {
  ...
  const matches = Object.values(ctx.worktrees).filter(
    (w) => w.branch.toLowerCase() === lower,
  );
