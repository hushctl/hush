...
export function generateVoronoiCells(
  ...
  const delaunay = Delaunay.from(
    { length: points.length / 2 } as ArrayLike<unknown>,
    (_, i) => points[i * 2],
    (_, i) => points[i * 2 + 1],
  );
  ...
  for (const [band, bandProjects] of byBand.entries()) {
    ...
    bandProjects.forEach(([, wts], bandIdx) => {
      ...
      for (let i = 0; i < wts.length; i++) {
        ...
        const y = Math.max(
          PADDING,
          Math.min(height - PADDING, startY + i * CLUSTER_V_GAP),
        );
        const dotSize =
          urgency === 0 ? 14 : urgency === 1 ? 11 : urgency === 2 ? 9 : 7;
      ...
      for (let j = i + 1; j < positions.length; j++) {
        ...
        if (dist < MIN_SEP && dist > 0.01) {
          ...
          positions[i].x = Math.max(
            PADDING,
            Math.min(width - PADDING, positions[i].x - nx * push * 0.2),
          );
          positions[i].y = Math.max(
            PADDING,
            Math.min(height - PADDING, positions[i].y - ny * push),
          );
          positions[j].x = Math.max(
            PADDING,
            Math.min(width - PADDING, positions[j].x + nx * push * 0.2),
          );
          positions[j].y = Math.max(
            PADDING,
            Math.min(height - PADDING, positions[j].y + ny * push),
          );
        }
      }
    }
