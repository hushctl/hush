...
export interface Panel {
  ...
  targetId: string;
  /** Shell session ID — only for kind === "shell". Each shell gets a unique ID. */
  shellId?: string;
  x: number;
  y: number;
  width: number;
...
export interface AppState {
  ...
  /** Last line of terminal output per worktree (Claude pty + shell). UI-only, not from daemon. */
  ...
  openPanel: (p: { kind: PanelKind; targetId: string; shellId?: string }) => void;
